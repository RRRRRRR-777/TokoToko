...
1. **確認は不要**: コミット前の承認確認は行わない
2. **addとcommitを同時実行**: `git add <file>` と `git commit -m "..."` を連続で実行
3. **git commit -aは使用しない**: 明示的に`git add`でファイルを指定してからコミット
...
1. **実行可能なコードの場合**
   - 構文チェックツールを実行（terraform fmt, go vet, swiftlint等）
   - エラーがあれば自動修正
   - 主要な変更点を説明
