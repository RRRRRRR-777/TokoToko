...
final class OnboardingManagerTests: XCTestCase {
  ...
  func testOnboardingManagerObservableObjectIntegration() {
    ...
    waitForExpectations(timeout: 1.0) { _ in
      XCTAssertEqual(
        notificationCount, 2,
        "ObservableObjectとして2回の変更通知が発生すること（currentContent + notificationTrigger）")
    }
  ...
  func testCheckVersionUpdateOnboardingWhenVersionUpdateNeeded() {
    ...
    XCTAssertEqual(
      content?.type, .versionUpdate(version: currentVersion), "正しいバージョンのオンボーディングタイプであること")
    XCTAssertFalse(content?.pages.isEmpty ?? true, "ページが含まれていること")
  }
  ...
  func testInitializationAutoSetsVersionUpdateOnboardingWhenFirstLaunchCompleted() {
    ...
    let shouldShowVersionUpdate = newManager.shouldShowOnboarding(
      for: .versionUpdate(version: currentVersion))

    if shouldShowVersionUpdate {
      XCTAssertTrue(hasContent, "バージョンアップデートが必要な場合、初期化時にcurrentContentが設定されること")
      XCTAssertEqual(
        newManager.currentContent?.type, .versionUpdate(version: currentVersion),
        "正しいバージョンアップデートタイプが設定されること")
    } else {
