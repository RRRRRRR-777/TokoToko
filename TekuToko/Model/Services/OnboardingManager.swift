...
class OnboardingManager: ObservableObject {
  ...
  func loadOnboardingFromYML(fileName: String) throws -> OnboardingConfig? {
    ...
    guard
      let url = Bundle.main.url(
        forResource: String(components[0]), withExtension: String(components[1]))
    else {
      throw OnboardingError.fileNotFound
    }
  ...
  private func createVersionUpdateContent(type: OnboardingType, version: String)
    -> OnboardingContent
  {
    // YMLから読み込んだデータを優先的に使用
    if let config = ymlConfig {
  ...
  private func findVersionSection(version: String, versionUpdates: [String: OnboardingSection])
    -> OnboardingSection?
  {
    // 完全一致を最初に試行
