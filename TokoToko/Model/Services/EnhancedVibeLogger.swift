...
public class EnhancedVibeLogger {
  ...
  public func adjustLogLevelBasedOnConditions() {
    logQueue.async { [weak self] in
      ...
      if memoryPressure > 0.8 {
        newLogLevel =
          LogLevel(rawValue: max(newLogLevel.rawValue, LogLevel.warning.rawValue)) ?? .warning
      }
  ...
  public func enableBatchLogging(batchSize: Int = 10, flushInterval: TimeInterval = 5.0) {
    ...
    batchTimer = Timer.scheduledTimer(withTimeInterval: flushInterval, repeats: true) {
      [weak self] _ in
      self?.flushLogBuffer()
    }
  }
