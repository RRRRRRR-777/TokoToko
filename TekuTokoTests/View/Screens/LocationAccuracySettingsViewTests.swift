...
final class LocationAccuracySettingsViewTests: XCTestCase {
  ...
  override func setUp() {
    ...
    mockUserDefaults = UserDefaults(suiteName: "com.TekuToko.test.settings")
    mockUserDefaults.removePersistentDomain(forName: "com.TekuToko.test.settings")
  ...
  func test_NavigationBarStyleManager統合_設定が正しく適用される() {
    ...
    // Then
    let navigationBar =
      hostingController.navigationController?.navigationBar ?? UINavigationBar.appearance()
    XCTAssertNotNil(
      navigationBar.standardAppearance, "NavigationBarStyleManagerによる設定が適用されている必要があります")
